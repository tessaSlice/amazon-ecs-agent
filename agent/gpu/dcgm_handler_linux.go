...
// read-only bind mount of the host's /var/run/ecs (created by the ecs-init
// package / a systemd-tmpfiles rule and written by dcgm-init), so any of these
// checks failing means dcgm-init has not produced metrics yet; that is an
// expected transient state, logged at Debug and surfaced as nil.

...
# /var/run/ecs (tmpfs, wiped on boot) is not created here or via tmpfiles. Both
# writers MkdirAll it at runtime before use — dcgm-init in engine.Start, ecs-init
# in getHostConfig before the bind mount — since there is no boot-ordering
# dependency between ecs.service and dcgm-init.service.

# ECS log directory (persistent root fs), so dcgm-init/agent/ecs-init can log
# from first start without the logger creating it lazily.
mkdir -p %{buildroot}%{_localstatedir}/log/ecs
...
# ECS log directory (holds dcgm-init.log, ecs-init.log, ecs-agent.log).
%dir %{_localstatedir}/log/ecs

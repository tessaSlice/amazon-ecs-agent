	...
	testCases := []struct {
	...
	}{
		{
			...
			metric: gputypes.GPUMetric{
				...
				TensorCoreUtilization: aws.Float64(40.0),
				SMActive:              aws.Float64(65.0),
			},
			expectNil:        false,
			expectedCount:    9,
			...
			expectedNames: []string{
				...
				"GPUMemoryUsed", "GPUPowerDraw", "GPUTemperature",
				"GPUTensorCoreUtilization", "GPUSMActive", "GPURestartAppXidCount",
			},
			expectedUnits: []string{
				"Percent", "Percent", "Bytes", "Bytes", "None", "None", "Percent", "Percent", "Count",
			},
			expectedIsDoubles: []bool{true, true, false, false, true, true, true, true, false},
		},
		{
			name: "only profiling fields set",
			metric: gputypes.GPUMetric{
				GPUUUID:               "GPU-prof-only",
				TensorCoreUtilization: aws.Float64(12.5),
				SMActive:              aws.Float64(88.0),
			},
			expectNil:         false,
			expectedCount:     3,
			expectedDimValue:  "GPU-prof-only",
			expectedNames:     []string{"GPUTensorCoreUtilization", "GPUSMActive", "GPURestartAppXidCount"},
			expectedUnits:     []string{"Percent", "Percent", "Count"},
			expectedIsDoubles: []bool{true, true, false},
		},
		{
			name: "all fields nil",
			metric: gputypes.GPUMetric{

...
var metricsFields = []metricsFieldDef{
	...
	{dcgm.DCGM_FI_DEV_MEM_COPY_UTIL, "DCGM_FI_DEV_MEM_COPY_UTIL"}, // fieldIdxMemUtil
...
type GPUMetric struct {
	...
	// DCGM field: DCGM_FI_DEV_MEM_COPY_UTIL.
...
func extractMetricsFromFieldValues(metric *GPUMetric, values []dcgm.FieldValue_v1) []string {
	...
	if fieldIdxMemUtil < len(values) {
		if isValidInt64FieldValue(values[fieldIdxMemUtil]) {
			v := float64(values[fieldIdxMemUtil].Int64())
